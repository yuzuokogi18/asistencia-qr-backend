...
class AsistenciaService {
     ...
     */
    static async escanearMatricula(matricula) {
        ...
        const horaActual = (0, date_util_1.getHoraActualStr)();
        // Buscar si ya existe registro de asistencia hoy
        const asistenciaHoy = await prisma_1.default.asistencia.findUnique({
            where: {
                alumno_id_fecha: {
                    alumno_id: alumno.id,
                    fecha: hoyDate,
        ...
        if (!asistenciaHoy || !asistenciaHoy.hora_entrada) {
            const esRetardo = (0, date_util_1.compareHoras)(horaActual, grupo.hora_limite_entrada) > 0;
            const estatus = esRetardo ? 'retardo' : 'a_tiempo';
            let alertaCreada = null;
            if (esRetardo) {
                alertaCreada = await alerta_service_1.AlertaService.crearAlerta({
                    alumno_id: alumno.id,
                    grupo_id: grupo.id,
                    tipo: 'retardo',
            ...
            const asistencia = await prisma_1.default.asistencia.upsert({
                where: {
                    alumno_id_fecha: {
                        alumno_id: alumno.id,
                        fecha: hoyDate,
        ...
        if (asistenciaHoy.hora_entrada && !asistenciaHoy.hora_salida) {
            ...
            const esSalidaAnticipada = (0, date_util_1.compareHoras)(horaActual, horaReferenciaSalida) < 0;
            let alertaCreada = null;
            ...
            const asistenciaActualizada = await prisma_1.default.asistencia.update({
    ...
    static async getAsistencias(filtros) {
        const whereClause = {};
        if (filtros.fecha) {
            whereClause.fecha = (0, date_util_1.parseFechaStr)(filtros.fecha);
        }
        ...
        const asistencias = await prisma_1.default.asistencia.findMany({
            where: whereClause,
            include: {
                alumno: {
                    select: {
    ...
    static async getResumenHoy() {
        ...
        const ayerStr = (0, date_util_1.getFechaActualStr)(ayer);
        const ayerDate = (0, date_util_1.parseFechaStr)(ayerStr);
        // Total de alumnos activos en el plantel
        const totalAlumnos = await prisma_1.default.alumno.count({
            where: { activo: true },
        });
        // Asistencias de hoy (alumnos que tienen hora_entrada)
        const asistenciasHoy = await prisma_1.default.asistencia.findMany({
            where: {
                fecha: hoyDate,
        ...
        const asistenciasAyer = await prisma_1.default.asistencia.findMany({
            where: {
                fecha: ayerDate,
                hora_entrada: { not: null },
            },
        ...
        const { lunes: lunesActual, viernes: viernesActual } = (0, date_util_1.getRangoSemanaLaboral)(new Date());
        ...
        const retardosSemanaActual = await prisma_1.default.asistencia.count({
            where: {
                fecha: { gte: lunesActual, lte: viernesActual },
                estatus: 'retardo',
            },
        });
        const retardosSemanaAnterior = await prisma_1.default.asistencia.count({
            where: {
     ...
     */
    static async getHistorialSemanal(fechaReferencia) {
        const refDate = fechaReferencia ? (0, date_util_1.parseFechaStr)(fechaReferencia) : new Date();
        const diasSemana = (0, date_util_1.getDiasLaboralesSemana)(refDate);
        ...
        for (let i = 0; i < diasSemana.length; i++) {
            ...
            const asistenciasDia = await prisma_1.default.asistencia.findMany({
                where: {
                    fecha: fechaDate,
                    hora_entrada: { not: null },
                },
     ...
     */
    static async cierreDiarioFaltas(fechaParam) {
        const fechaStr = fechaParam || (0, date_util_1.getFechaActualStr)();
        const fechaDate = (0, date_util_1.parseFechaStr)(fechaStr);
        const alumnosActivos = await prisma_1.default.alumno.findMany({
            where: { activo: true },
            include: {
                grupo: true,
                asistencias: {
        ...
        for (const alumno of alumnosActivos) {
            ...
            if (!tieneAsistencia) {
                ...
                await prisma_1.default.asistencia.upsert({
                    where: {
                        alumno_id_fecha: {
                            alumno_id: alumno.id,
                            fecha: fechaDate,
                ...
                await alerta_service_1.AlertaService.crearAlerta({
                    alumno_id: alumno.id,
